---
description: "NodeAuth 项目初始化与激活教程。引导您通过 Smart Shield 智能自检系统排除环境变量风险，完成首次登录。重点讲解如何生成并验证“应急恢复包 (PDF)”，确保核心加密密钥的安全离线保存，并开启 PIN 码锁定与防窥模式等初步安全设置。"
---
# 初始化教程
...
### 1. 核心安全自检项
*   **系统授权码 (NODEAUTH_LICENSE)**：检测授权码是否配置。您可以从 [license.nodeauth.io](https://license.nodeauth.io) 获取。
*   **核心加密密钥 (ENCRYPTION_KEY)**：检测密钥是否为空或长度不足 32 位。这是加密数据库的“命根子”，不达标将直接阻断访问。
*   **会话签名密钥 (JWT_SECRET)**：检测签名密钥强度。如果密钥过于简单，可能会导致黑客伪造身份。
*   **登录白名单 (OAUTH_ALLOWED_USERS)**：检测是否配置了允许登录的邮箱、Telegram ID、Web3 钱包地址。多个值请用英文逗号分开。
