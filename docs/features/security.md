---
description: "NodeAuth 极致安全架构详解。深入探讨零知识架构 (Zero-Knowledge)、端对端加密 (E2EE) 以及全库冷加密机制。涵盖硬件盐注入、生物识别准入控制、防窥模式 (Ghost Mode) 及 Air-Gap 离线隔离等核心安全技术，为您构建不可逾越的数字身份防线。"
---
# 🛡️ 极致安全：全方位隐私防线
...
NodeAuth 遵循“不触碰、不窥探、不持有”的核心原则。通过将浏览器环境转化为“主密码”，确保即便服务器管理员也物理上无法看到您的 2FA 密钥。

---

## 🔐 2. 端对端加密 (End-to-End Encryption)
核心资产在离开浏览器前即已加密成 `nodeauth:` 密文。整个网络传输与存储生命周期中，数据仅在您的设备端可被解开，实现了商业级密码管理器的保护标准。

---

## ❄️ 3. 全库冷加密架构 (Cold Encryption)
不同于传统应用将 Secret 明文存放在数据库，NodeAuth 实现了**全库冷处理**：
*   **本地 AES-GCM 封装**：所有敏感数据在本地 IndexedDB 中均以加密 BLOB 形式存在。
*   **核心逻辑锁**：系统底层通过 `NODEAUTH_LICENSE` 实现算法级的“逻辑注入”，确保加解密链路与您的授权身份物理绑定，即使环境变量泄露也能防范解密。
*   **硬件盐注入**：利用 WebAuthn 派生的设备指纹作为加密锚点 (Device Salt)，即使数据库文件被打包窃取，换一台设备也无法轻易解密。
*   **密钥不落地**：核心解密密钥仅在内存中存活，系统注销或刷新页面后即刻被擦除，杜绝物理取证。

---

## 🔒 4. 实时访问控制与物理门禁 (AppLock)
极致的安全需要“门禁”级的准入控制：
